...
sealed class Screen(val route: String) {
    ...
    object AddHabit : Screen("add_habit?title={title}&categoryId={categoryId}") {
        fun createRoute(title: String? = null, categoryId: String? = null): String {
            return when {
                title != null && categoryId != null -> "add_habit?title=${Uri.encode(title)}&categoryId=${Uri.encode(categoryId)}"
                title != null -> "add_habit?title=${Uri.encode(title)}"
                categoryId != null -> "add_habit?categoryId=${Uri.encode(categoryId)}"
                else -> "add_habit"
            }
        }
    }
    ...
    object IdeaCategoryDetail : Screen("idea_category/{categoryId}") {
        fun createRoute(categoryId: String) = "idea_category/${Uri.encode(categoryId)}"
    }
